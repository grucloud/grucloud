...
const createResources = async ({ provider }) => {
  ...
  const rhmig = await provider.compute.makeNetwork({
    name: "rhmig",
    properties: () => ({ autoCreateSubnetworks: false }),
  });

  const subnet_10_0_0_0_16 = await provider.compute.makeSubNetwork({
  ...
  const extraDisk = await provider.compute.makeDisk({
    name: "extra-disk",
    properties: () => ({}),
  });

  const s1_2Uk1 = await provider.compute.makeVmInstance({
    name: "s1-2-uk1",
