...
```js
const s3Accelerated = await provider.s3.makeBucket({
  name: `yourgloballyuniquebucketnamehere`,
  properties: () => ({
...
```js
const s3Encryption = await provider.s3.makeBucket({
  name: `yourgloballyuniquebucketnamehere`,
  properties: () => ({
...
```js
const s3Lifecycle = await provider.s3.makeBucket({
  name: `yourgloballyuniquebucketnamehere`,
  properties: () => ({
...
const s3pPolicy = await provider.s3.makeBucket({
  name: bucketName,
  properties: () => ({
...
const s3Replication = await provider.s3.makeBucket({
  name: bucketName,
  properties: () => ({
...
```js
const s3RequestPayment = await provider.s3.makeBucket({
  name: `yourgloballyuniquebucketnamehere`,
  properties: () => ({
